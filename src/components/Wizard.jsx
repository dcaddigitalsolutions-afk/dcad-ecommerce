...
const C={red:"#E52229",dark:"#111",dark2:"#1A1A1A",dark3:"#2A2A2A",border:"rgba(255,255,255,0.08)",text:"#FFF",textSec:"#B8B8B8",textMut:"#444",green:"#22C55E",greenBg:"rgba(34,197,94,0.08)",yellow:"#EAB308",blue:"#6366F1"};
...
export default function App() {
  ...
  const [isDesktop,setIsDesktop]=useState(false); // field key being loaded
  ...
  return (
    <div style={{minHeight:"100vh",background:C.dark,fontFamily:"system-ui,sans-serif",color:C.text,position:"relative"}}>
      ...
      {/* ── GLOBAL FOCUS RING ── */}
      <style>{`*:focus-visible{outline:2px solid #E52229 !important;outline-offset:2px;border-radius:4px;}button,a,[role="button"]{cursor:pointer;}`}</style>
      ...
      {printerModal&&(
        <div onClick={()=>setPrinterModal(null)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.85)",zIndex:65,display:"flex",alignItems:"center",justifyContent:"center",padding:20,minHeight:"100vh"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#1A1A1A",border:"1px solid rgba(13,148,136,0.4)",borderRadius:14,padding:24,maxWidth:480,width:"100%",maxHeight:"85vh",overflowY:"auto"}}>
            ...
            {[["apelido","Apelido / Nome interno","Ex: Impressora guias"],["marca","Marca da impressora *","Ex: Creality, Phrozen, Anycubic"],["modelo","Modelo *","Ex: Halot One, Sonic Mini 8K"],["resina_marca","Marca da resina *","Ex: Saremco, Keystone, SprintRay"],["resina_modelo","Modelo da resina *","Ex: Guide Clear, Model Resin Pro"]].map(([k,label,ph])=>(
              <div key={k} style={{marginBottom:12}}>
                <label style={{fontSize:11,fontWeight:700,color:C.textSec,display:"block",marginBottom:5,letterSpacing:.5}}>{label.toUpperCase()}</label>
                <input value={printerForm[k]} onChange={e=>setPrinterForm(f=>({...f,[k]:e.target.value}))} placeholder={ph}
                  style={{width:"100%",background:"#222",border:`1px solid ${printerForm[k]?"rgba(13,148,136,0.4)":C.border}`,borderRadius:10,padding:"11px 14px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
              </div>
            ))}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
              <div>
                <label style={{fontSize:11,fontWeight:700,color:C.textSec,display:"block",marginBottom:5,letterSpacing:.5}}>ALTURA DE CAMADA (mm) *</label>
                <input value={printerForm.camada} onChange={e=>setPrinterForm(f=>({...f,camada:e.target.value}))} placeholder="Ex: 0.05" type="number" step="0.01" min="0.01"
                  style={{width:"100%",background:"#222",border:`1px solid ${printerForm.camada?"rgba(13,148,136,0.4)":C.border}`,borderRadius:10,padding:"11px 14px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
              </div>
              <div>
                <label style={{fontSize:11,fontWeight:700,color:C.textSec,display:"block",marginBottom:5,letterSpacing:.5}}>TEMPO DE EXPOSIÇÃO (s) *</label>
                <input value={printerForm.exposicao} onChange={e=>setPrinterForm(f=>({...f,exposicao:e.target.value}))} placeholder="Ex: 8" type="number" step="0.5" min="0.5"
                  style={{width:"100%",background:"#222",border:`1px solid ${printerForm.exposicao?"rgba(13,148,136,0.4)":C.border}`,borderRadius:10,padding:"11px 14px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
              </div>
            </div>
            <div style={{marginBottom:18}}>
              <label style={{fontSize:11,fontWeight:700,color:C.textSec,display:"block",marginBottom:8,letterSpacing:.5}}>TIPOS DE SERVIÇO ONDE USAR ESTA IMPRESSORA</label>
              {["Guia Cirúrgico","Placa Oclusal","Modelo de Estudo","Perioguide","Guia Endodôntico","Outros"].map(st=>{
                ...
                return(
                  <button key={st} onClick={()=>setPrinterForm(f=>({...f,serviceTypes:sel?f.serviceTypes.filter(x=>x!==st):[...f.serviceTypes,st]}))}
                    style={{display:"inline-flex",alignItems:"center",gap:6,background:sel?"rgba(13,148,136,0.15)":"rgba(255,255,255,0.04)",border:`1px solid ${sel?"rgba(13,148,136,0.4)":C.border}`,borderRadius:20,padding:"5px 12px",cursor:"pointer",color:sel?"#0D9488":C.textSec,fontSize:12,fontWeight:sel?700:400,marginRight:6,marginBottom:6}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setPrinterModal(null)} style={{flex:1,padding:"11px 0",background:"transparent",border:`1px solid ${C.border}`,borderRadius:10,color:C.textSec,fontSize:13,cursor:"pointer"}}>Cancelar</button>
              <button disabled={!printerForm.marca||!printerForm.modelo||!printerForm.resina_marca||!printerForm.resina_modelo||!printerForm.camada||!printerForm.exposicao}
                ...
                style={{flex:2,padding:"11px 0",background:"rgba(13,148,136,0.15)",border:"1px solid rgba(13,148,136,0.4)",borderRadius:10,color:"#0D9488",fontSize:13,fontWeight:700,cursor:"pointer",opacity:(!printerForm.marca||!printerForm.modelo||!printerForm.resina_marca||!printerForm.resina_modelo||!printerForm.camada||!printerForm.exposicao)?0.4:1}}>
      ...
      {confirmDeleteId&&(
        <div onClick={()=>setConfirmDeleteId(null)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.88)",zIndex:70,display:"flex",alignItems:"center",justifyContent:"center",padding:20,minHeight:"100vh"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#1A1A1A",border:"1px solid rgba(229,34,41,0.35)",borderRadius:16,padding:28,maxWidth:400,width:"100%"}}>
            <div style={{display:"flex",justifyContent:"center",marginBottom:14}}>
              <div style={{width:52,height:52,borderRadius:14,background:"rgba(229,34,41,0.1)",border:"1px solid rgba(229,34,41,0.3)",display:"flex",alignItems:"center",justifyContent:"center"}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setConfirmDeleteId(null)}
                style={{flex:1,padding:"13px 0",background:"none",border:`1px solid ${C.border}`,borderRadius:10,color:C.textSec,fontSize:14,fontWeight:600,cursor:"pointer"}}>
                Cancelar
              </button>
              <button onClick={()=>{
                ...
                style={{flex:1,padding:"13px 0",background:"rgba(229,34,41,0.12)",border:"1px solid rgba(229,34,41,0.4)",borderRadius:10,color:C.red,fontSize:14,fontWeight:700,cursor:"pointer"}}>
                Sim, remover
              </button>
            </div>
          </div>
      ...
      {showStackableInfo&&(
        <div onClick={()=>{setShowStackableInfo(false);setScreen(7);}} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.88)",zIndex:70,display:"flex",alignItems:"center",justifyContent:"center",padding:20,minHeight:"100vh"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#1A1A1A",border:"1px solid rgba(99,102,241,0.35)",borderRadius:16,padding:28,maxWidth:420,width:"100%"}}>
            <div style={{display:"flex",justifyContent:"center",marginBottom:14}}>
              <div style={{width:52,height:52,borderRadius:14,background:"rgba(99,102,241,0.1)",border:"1px solid rgba(99,102,241,0.3)",display:"flex",alignItems:"center",justifyContent:"center"}}>
            ...
            <div style={{background:"rgba(99,102,241,0.06)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:10,padding:"12px 14px",marginBottom:20}}>
              <div style={{fontSize:11,fontWeight:700,color:C.blue,marginBottom:8,letterSpacing:.5}}>✓ INCLUSO NO SERVIÇO</div>
            ...
            <button onClick={()=>{setShowStackableInfo(false);setScreen(7);}}
              style={{width:"100%",padding:"13px 0",background:"rgba(99,102,241,0.12)",border:"1px solid rgba(99,102,241,0.4)",borderRadius:10,color:C.blue,fontSize:14,fontWeight:700,cursor:"pointer"}}>
      ...
      {showProtoWarning&&(
        <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.88)",zIndex:70,display:"flex",alignItems:"center",justifyContent:"center",padding:20,minHeight:"100vh"}}>
          <div style={{background:"#1A1A1A",border:"1px solid rgba(234,179,8,0.4)",borderRadius:16,padding:28,maxWidth:460,width:"100%"}}>
            {/* Ícone */}
            <div style={{display:"flex",justifyContent:"center",marginBottom:14}}>
              <div style={{width:52,height:52,borderRadius:14,background:"rgba(234,179,8,0.1)",border:"1px solid rgba(234,179,8,0.35)",display:"flex",alignItems:"center",justifyContent:"center"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <button onClick={()=>{setShowProtoWarning(false);setScreen(2);}}
                style={{width:"100%",padding:"13px 0",background:"rgba(234,179,8,0.12)",border:"1px solid rgba(234,179,8,0.4)",borderRadius:10,color:C.yellow,fontSize:14,fontWeight:700,cursor:"pointer"}}>
                Entendido — continuar sem o planejamento
              </button>
              <button onClick={()=>{setShowProtoWarning(false);setA(a=>({...a,dsd3dAddon:true}));}}
                style={{width:"100%",padding:"13px 0",background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.25)",borderRadius:10,color:C.green,fontSize:13,fontWeight:600,cursor:"pointer"}}>
      ...
      {showPreplanConfirm&&(
        <div onClick={()=>setShowPreplanConfirm(false)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.85)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",padding:20,minHeight:"100vh"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:C.dark2,border:"1px solid rgba(234,179,8,0.4)",borderRadius:14,padding:24,maxWidth:500,width:"100%",maxHeight:"80vh",overflowY:"auto"}}>
            ...
            <pre style={{fontSize:13,color:C.text,lineHeight:1.8,whiteSpace:"pre-wrap",fontFamily:"inherit",margin:"0 0 14px 0",background:"rgba(234,179,8,0.06)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:10,padding:"12px 14px"}}>{PREPLAN_RULES}</pre>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setShowPreplanConfirm(false)} style={{flex:1,padding:"11px 0",background:C.dark3,border:`1px solid ${C.border}`,borderRadius:10,color:C.textSec,fontSize:13,cursor:"pointer"}}>
                Voltar
              </button>
              <button onClick={()=>{setShowPreplanConfirm(false);set("preplan","sim");setScreen(7);}} style={{flex:1,padding:"11px 0",background:"rgba(234,179,8,0.15)",border:"1px solid rgba(234,179,8,0.4)",borderRadius:10,color:"#FCD34D",fontSize:13,fontWeight:700,cursor:"pointer"}}>
                Entendido — confirmar
              </button>
            </div>
          </div>
      ...
      {dentist&&screen!=="auth"&&screen!=="dashboard"&&(
        <a href={`https://wa.me/5598985425982?text=${encodeURIComponent(`Olá! Sou ${dentist.nome} (${dentist.cro}). Preciso de ajuda com: ${specialty?specialty.name+' — ':''} ${service?service.name:''}`)}`}
          ...
          style={{position:"fixed",bottom:24,right:24,zIndex:150,width:48,height:48,borderRadius:24,background:"#22C55E",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px rgba(34,197,94,0.4)",textDecoration:"none",transition:"transform .2s"}}
      ...
      {undoService&&(
        <div style={{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",zIndex:200,background:C.dark2,border:`1px solid ${C.border}`,borderRadius:12,padding:"10px 16px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 4px 24px rgba(0,0,0,0.5)",whiteSpace:"nowrap"}}>
          ...
          <button onClick={()=>{setServices(s=>[...s,undoService.service]);clearTimeout(undoService.timer);setUndoService(null);}}
            style={{display:"flex",alignItems:"center",gap:5,background:"rgba(99,102,241,0.15)",border:"1px solid rgba(99,102,241,0.3)",borderRadius:7,padding:"5px 10px",color:C.blue,fontSize:12,fontWeight:700,cursor:"pointer"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          {dentist&&screen!=="auth"&&screen!=="dashboard"&&screen!=="pending"&&(
            <button onClick={()=>setScreen("dashboard")}
              style={{display:"flex",alignItems:"center",gap:5,background:"none",border:`1px solid ${C.border}`,color:C.textSec,fontSize:11,padding:"4px 10px",borderRadius:6,cursor:"pointer",marginLeft:4}}>
      ...
      {editingId&&(
        <div style={{background:"rgba(99,102,241,0.1)",borderBottom:"1px solid rgba(99,102,241,0.2)",padding:"7px 20px",display:"flex",alignItems:"center",gap:8,fontSize:12,color:C.blue}}>
          ...
          <button onClick={()=>{
            ...
            setEditingId(null);
            setScreen("resumo");
          }} style={{marginLeft:"auto",background:"none",border:"none",color:C.textSec,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>Cancelar edição</button>
        </div>
      )}
      ...
      {stepNum>=0&&(
        <div style={{background:C.dark2,padding:"8px 20px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
            <div style={{flex:1,height:3,background:C.dark3,borderRadius:2,overflow:"hidden"}}>
              <div style={{height:"100%",width:(stepNum/(WIZARD_STEPS.length-1)*100)+"%",background:C.red,transition:"width .4s ease"}}/>
            </div>
            <span style={{fontSize:11,color:C.textSec,whiteSpace:"nowrap",fontWeight:600,flexShrink:0}}>
              {stepNum+1}/{WIZARD_STEPS.length}
            </span>
          </div>
          <div style={{display:"flex",gap:4,flexWrap:"wrap",justifyContent:"center"}}>
            {WIZARD_STEPS.map((l,i)=>(
              <span key={i} style={{fontSize:10,padding:"2px 8px",borderRadius:20,background:i===stepNum?C.red:i<stepNum?"rgba(229,34,41,0.15)":C.dark3,color:i===stepNum?"#fff":i<stepNum?C.red:C.textSec,fontWeight:i===stepNum?700:400,transition:"all .2s",cursor:i<stepNum?"pointer":"default"}}
      ...
      <div suppressHydrationWarning style={{display:"flex",alignItems:"flex-start",width:"100%"}}>
        ...
        {isDesktop&&dentist&&screen!=="auth"&&screen!=="dashboard"&&(patient||specialty||services.length>0)&&(
          <div style={{width:256,flexShrink:0,borderRight:`1px solid ${C.border}`,minHeight:"calc(100vh - 52px)",padding:"16px 14px",position:"sticky",top:52,overflowY:"auto",maxHeight:"calc(100vh - 52px)"}}>
            ...
            {/* Section title */}
            <div style={{fontSize:11,fontWeight:700,color:C.textSec,letterSpacing:1.5,marginBottom:12}}>RESUMO DO CASO</div>
            ...
            {[
              ...
              <div key={i} style={{display:"flex",gap:8,marginBottom:8,alignItems:"flex-start",opacity:item.active?1:0.4}}>
                <div style={{width:22,height:22,borderRadius:6,background:item.active?"rgba(229,34,41,0.1)":"rgba(255,255,255,0.04)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,flexShrink:0,marginTop:1}}>
                  {item.active?item.icon:"○"}
                </div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:4}}>
                    <span style={{fontSize:11,fontWeight:700,color:C.textSec,letterSpacing:.5,textTransform:"uppercase"}}>{item.label}</span>
                    {item.action&&<button onClick={item.action} style={{background:"none",border:"none",color:C.red,fontSize:11,cursor:"pointer",padding:0,flexShrink:0}}>{item.actionLabel}</button>}
                  </div>
                  {item.value&&<div style={{fontSize:11,fontWeight:600,color:C.text,lineHeight:1.3,wordBreak:"break-word",marginTop:1}}>{item.value}</div>}
                </div>
              </div>
            ))}
            ...
            {specialty&&service&&screen!=="resumo"&&screen!=="checkout"&&screen!=="auth"&&screen!=="dashboard"&&(
              <div style={{borderTop:`1px solid ${C.border}`,marginTop:8,paddingTop:10,marginBottom:4}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                  <span style={{fontSize:10,color:C.textSec,fontWeight:700,letterSpacing:.5}}>SUBTOTAL</span>
                  <span style={{fontSize:15,fontWeight:900,color:C.red}}>{fmt(calcTotal(A))}</span>
                </div>
                {services.length>0&&(
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:10,color:C.textSec}}>+ {services.length} serviço{services.length>1?"s":""} no carrinho</span>
                    <span style={{fontSize:11,fontWeight:700,color:C.textSec}}>{fmt(totalAll)}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {services.length>0&&(
              <div>
                ...
                <div style={{fontSize:11,fontWeight:700,color:C.textSec,letterSpacing:1,marginBottom:8}}>CARRINHO ({services.length})</div>
                {services.map(sv=>(
                  <div key={sv.id} style={{background:C.dark2,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 10px",marginBottom:5}}>
                    <div style={{fontWeight:700,fontSize:11,marginBottom:2,lineHeight:1.3}}>{sv.name}</div>
                    <div style={{fontSize:10,color:C.textSec}}>{sv.teeth?.join(", ")||"—"} · {sv.brand||"—"}</div>
                    <div style={{fontWeight:800,fontSize:11,color:C.red,marginTop:3}}>{fmt(sv.total)}</div>
                  </div>
                ))}
                <div style={{borderTop:`1px solid ${C.border}`,paddingTop:7,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:10,color:C.textSec}}>Total</span>
            ...
            {/* Discount badge */}
            {discount>0&&(
              <div style={{background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:8,padding:"7px 9px",fontSize:10,color:C.green,fontWeight:700,marginTop:10,textAlign:"center"}}>
      ...
      <div suppressHydrationWarning style={W}>
                    ...
                    <div style={{borderTop:`1px solid ${C.border}`,margin:"16px 0 14px",paddingTop:14}}>
                      ...
                      {(regForm.tipo==="clinica"||regForm.tipo==="cursos")
                        ?<div style={{background:C.dark3,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 13px",marginBottom:12,fontSize:13,color:C.textSec}}>
                            CNPJ <span style={{fontSize:11,color:C.textSec,fontWeight:400}}>— obrigatório para este tipo de conta</span>
                          </div>
          ...
          <div style={{maxWidth:560,margin:"0 auto",paddingTop:isDesktop?32:8}}>
            ...
            <div style={{padding:"14px 16px",background:"rgba(13,148,136,0.06)",border:"1px solid rgba(13,148,136,0.25)",borderRadius:12,marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                <div>
                  <div style={{fontSize:11,fontWeight:700,color:"#0D9488",letterSpacing:1}}>IMPRESSORAS 3D CADASTRADAS</div>
                  <div style={{fontSize:11,color:C.textSec,marginTop:2}}>Seus parâmetros são preenchidos automaticamente ao contratar Somente Planejamento.</div>
                </div>
              ...
              {printers.map((pr,i)=>(
                <div key={pr.id} style={{background:C.dark2,border:`1px solid ${C.border}`,borderRadius:10,padding:"11px 13px",marginBottom:8,display:"flex",alignItems:"flex-start",gap:10}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:11,color:C.textSec,marginTop:2}}>Resina: {pr.resina_marca} {pr.resina_modelo}</div>
                    <div style={{fontSize:11,color:C.textSec}}>Camada: {pr.camada}mm · Exposição: {pr.exposicao}s</div>
                    {pr.serviceTypes?.length>0&&<div style={{fontSize:10,color:"#0D9488",marginTop:3,fontWeight:600}}>Usada em: {pr.serviceTypes.join(", ")}</div>}
                  </div>
                  <div style={{display:"flex",gap:6,flexShrink:0}}>
                    <button onClick={()=>{setPrinterForm({...pr});setPrinterModal(pr);}}
                      style={{background:"rgba(99,102,241,0.1)",border:"1px solid rgba(99,102,241,0.3)",borderRadius:7,padding:"5px 10px",color:C.blue,fontSize:11,cursor:"pointer",fontWeight:600}}>Editar</button>
                    <button onClick={()=>setPrinters(p=>p.filter((_,j)=>j!==i))}
                      style={{background:"rgba(229,34,41,0.08)",border:"1px solid rgba(229,34,41,0.2)",borderRadius:7,padding:"5px 8px",color:C.red,fontSize:11,cursor:"pointer"}}>
                      <Trash2 size={12}/>
                    </button>
                  </div>
                </div>
        ...
        {screen==="dashboard"&&(
          <div style={{width:"100%"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {[
                  ...
                  <button key={i} onClick={a.action}
                    style={{display:"flex",alignItems:"center",gap:12,background:C.dark2,border:`1px solid ${C.border}`,borderRadius:12,padding:"13px 14px",cursor:"pointer",textAlign:"left",color:C.text,transition:"border-color .15s"}}
                    onMouseEnter={e=>e.currentTarget.style.borderColor="#555"}
                    onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>
                    <div style={{width:38,height:38,borderRadius:10,background:a.clr,display:"flex",alignItems:"center",justifyContent:"center",color:C.text,flexShrink:0}}>{a.icon}</div>
                    <div>
            ...
            {pendingCarts.length>0&&(
              <div style={{marginBottom:24}}>
                <div style={{fontSize:11,fontWeight:700,color:C.yellow,letterSpacing:1,marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {pendingCarts.map((cart,i)=>(
                    <div key={i} style={{background:C.dark2,border:"1px solid rgba(234,179,8,0.25)",borderRadius:12,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
                      <div style={{width:38,height:38,borderRadius:10,background:"rgba(234,179,8,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}><ShoppingCart size={20}/></div>
                      <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{textAlign:"right",flexShrink:0}}>
                        ...
                        <button onClick={()=>{
                          ...
                          setScreen("resumo");
                        }} style={{background:"rgba(234,179,8,0.12)",border:"1px solid rgba(234,179,8,0.3)",borderRadius:7,padding:"4px 10px",color:C.yellow,fontSize:11,fontWeight:700,cursor:"pointer"}}>
                          Finalizar pedido →
                        </button>
                      </div>
                    </div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {pendingPayments.map((o,i)=>(
                    <div key={o.id||i} style={{background:C.dark2,border:"1px solid rgba(234,179,8,0.3)",borderRadius:12,padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
                      <div style={{width:38,height:38,borderRadius:10,background:"rgba(234,179,8,0.1)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}><Clock size={18} color={C.yellow}/></div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontWeight:700,fontSize:13,marginBottom:2}}>{o.servicesSummary}</div>
                        <div style={{fontSize:11,color:C.textSec}}>Paciente: {o.patientNome} · {o.date}</div>
                      </div>
                      <div style={{textAlign:"right",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                        <div style={{fontWeight:800,fontSize:14,color:C.yellow}}>{fmt(o.total)}</div>
                        <div style={{fontSize:10,background:"rgba(234,179,8,0.12)",color:C.yellow,padding:"2px 7px",borderRadius:20,fontWeight:700}}>Aguard. pagamento</div>
                        <button onClick={()=>window.open(o.checkoutUrl,"_blank")}
                          style={{background:C.yellow,border:"none",borderRadius:7,padding:"5px 12px",color:"#111",fontSize:11,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>
                          Finalizar pagamento →
                        </button>
                      </div>
                    </div>
                    ...
                    <div key={sv.id} style={{background:C.dark2,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",display:"flex",flexDirection:"column",gap:6}}>
                      <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <div style={{fontSize:10,color:C.textSec}}>{patient?.nome||"—"}</div>
                        </div>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <div style={{fontSize:10,background:"rgba(234,179,8,0.12)",color:C.yellow,padding:"2px 7px",borderRadius:20,fontWeight:700}}>Aguard. pagamento</div>
                      ...
                      <div key={o.id} style={{background:C.dark2,border:`1px solid ${o.status==="pending_payment"?"rgba(234,179,8,0.3)":C.border}`,borderRadius:12,padding:"12px 14px",display:"flex",flexDirection:"column",gap:6}}>
                        <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{fontSize:10,color:C.textSec}}>{o.patient} · {o.date}</div>
                          </div>
                        </div>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:6}}>
                          <div style={{fontSize:10,background:s.bg,color:s.color,padding:"2px 7px",borderRadius:20,fontWeight:700,whiteSpace:"nowrap"}}>{s.label}</div>
                        ...
                        {o.status==="pending_payment"&&(
                          <button onClick={()=>{
                            ...
                            setCouponDiscount(0);
                            setScreen("resumo");
                          }} style={{background:C.yellow,border:"none",borderRadius:7,padding:"5px 0",color:"#111",fontSize:10,fontWeight:800,cursor:"pointer",width:"100%"}}>
                            Finalizar pagamento →
                          </button>
                        )}
                        {o.status!=="pending_payment"&&o.filesSubmitted===false&&(
                          <a href={process.env.NEXT_PUBLIC_CLICKUP_FORM_URL||"#"} target="_blank" rel="noopener noreferrer"
                            style={{background:"rgba(99,102,241,0.12)",border:"1px solid rgba(99,102,241,0.3)",borderRadius:7,padding:"5px 0",color:C.blue,fontSize:10,fontWeight:700,textDecoration:"none",display:"block",textAlign:"center",width:"100%"}}>
        ...
        {screen==="patient"&&(
          <div style={{maxWidth:560,margin:"0 auto",paddingTop:isDesktop?32:8}}>
                ...
                {savedPatients
                    ...
                    <button key={p.id} onClick={()=>{
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        {p.nasc&&<div style={{fontSize:11,color:C.textSec,marginTop:1}}>{calcAge(p.nasc)?`${calcAge(p.nasc)} · Nasc. ${p.nasc}`:p.nasc}</div>}
                      </div>
            ...
            <div style={{marginBottom:12}}>
              <label style={{fontSize:11,fontWeight:700,color:C.textSec,display:"block",marginBottom:5,letterSpacing:.5}}>Data de nascimento *</label>
              <input value={patForm.nasc} onChange={e=>setPatForm(f=>({...f,nasc:maskDob(e.target.value)}))} placeholder="dd/mm/aaaa" inputMode="numeric" maxLength={10}
                style={{width:"100%",background:C.dark2,border:`1px solid ${patForm.nasc&&!calcAge(patForm.nasc)?C.red:patForm.nasc?"rgba(99,102,241,0.4)":C.border}`,borderRadius:10,padding:"11px 14px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",transition:"border-color .2s"}}/>
              {patForm.nasc&&calcAge(patForm.nasc)&&<div style={{fontSize:11,color:C.green,marginTop:5,fontWeight:600}}>Idade: {calcAge(patForm.nasc)}</div>}
              {patForm.nasc.length===10&&!calcAge(patForm.nasc)&&<div style={{fontSize:11,color:C.red,marginTop:5}}>Data inválida. Verifique dia, mês e ano.</div>}
            </div>
              ...
              {CATALOG.map((sp)=>(
                <button key={sp.id}
                  ...
                  <div style={{width:"100%",aspectRatio:"1/1",background:`${sp.color}12`,display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",flexShrink:0,position:"relative"}}>
                    ...
                    {discount>0&&<div style={{position:"absolute",top:4,right:4,fontSize:8,fontWeight:700,background:"rgba(34,197,94,0.92)",color:"#fff",padding:"1px 5px",borderRadius:20}}>-15%</div>}
                  </div>
                  <div style={{padding:"10px 10px 12px",background:"rgba(0,0,0,0.65)",flexShrink:0}}>
                    ...
                    <div style={{fontSize:11,fontWeight:700,color:sp.color}}>{sp.services.length} serviço{sp.services.length===1?"":"s"}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        {screen==="services"&&specialty&&(
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              <div style={{width:36,height:36,borderRadius:10,background:specialty.color+"20",display:"flex",alignItems:"center",justifyContent:"center",color:specialty.color}}><SpecIcon id={specialty.id} size={20} color={specialty.color}/></div>
              <div style={{flex:1}}>
            ...
            {serviceType&&(
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:serviceType==="print"?"rgba(34,197,94,0.06)":"rgba(234,179,8,0.06)",border:`1px solid ${serviceType==="print"?"rgba(34,197,94,0.2)":"rgba(234,179,8,0.2)"}`,borderRadius:10,padding:"8px 12px",marginBottom:20}}>
              ...
              return groups.map(group=>{
                ...
                return(
                  <button key={group.type}
                    ...
                    style={{width:"100%",display:"flex",flexDirection:"column",background:C.dark2,border:`1.5px solid ${C.border}`,borderRadius:14,padding:0,cursor:"pointer",textAlign:"left",marginBottom:12,color:C.text,overflow:"hidden",transition:"border-color .15s,background .15s"}}
                    ...
                    <div style={{padding:"16px 16px",display:"flex",gap:14,alignItems:"flex-start"}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,marginBottom:singleSv?6:4}}>
                          ...
                          <div style={{textAlign:"right",flexShrink:0}}>
                            {hasVariablePrice&&<div style={{fontSize:10,color:C.textSec,marginBottom:2}}>a partir de</div>}
                            {isArchBased&&<div style={{fontSize:10,color:C.textSec,marginBottom:2}}>por arcada</div>}
                            <div style={{fontWeight:900,fontSize:15,color:specialty.color}}>{fmt(applyDiscount(minPrice))}</div>
                          </div>
                        </div>
                        {/* Subtítulo do grupo */}
                        <div style={{fontSize:11,color:C.textSec,lineHeight:1.5,marginBottom:(singleSv||isParcialGroup)&&8}}>
                            ...
                            <div style={{display:"grid",gridTemplateColumns:hasIncludes?"1fr 1fr":"1fr",gap:8,marginBottom:4}}>
                              ...
                              {sv0.exams?.length>0&&(
                                <div style={{background:"rgba(99,102,241,0.06)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:8,padding:"7px 10px"}}>
                                  <div style={{fontSize:10,fontWeight:700,color:C.blue,letterSpacing:.5,marginBottom:5,display:"flex",alignItems:"center",gap:4}}>
                                    <FileText size={10} color={C.blue}/> ARQUIVOS NECESSÁRIOS
                                  </div>
                                  {sv0.exams.map((ex,ei)=>(
                                    <div key={ei} style={{fontSize:11,color:C.textSec,display:"flex",alignItems:"flex-start",gap:5,marginBottom:3}}>
                                      <span style={{color:C.blue,flexShrink:0}}>·</span>{ex}
                                    </div>
                                  ))}
                                  <a href={EXAMS_MANUAL_URL} target="_blank" rel="noopener noreferrer"
                                    onClick={e=>e.stopPropagation()}
                                    style={{display:"inline-flex",alignItems:"center",gap:4,marginTop:6,background:"rgba(99,102,241,0.08)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:20,padding:"3px 9px",textDecoration:"none",cursor:"pointer"}}>
                                    <Search size={9} color={C.blue}/>
                                    <span style={{fontSize:9,fontWeight:700,color:C.blue}}>DÚVIDAS? CLIQUE AQUI</span>
                                  </a>
                                </div>
                              )}
                              {/* O que está incluso */}
                              {hasIncludes&&(
                                <div style={{background:"rgba(34,197,94,0.05)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:8,padding:"7px 10px"}}>
                                  <div style={{fontSize:10,fontWeight:700,color:C.green,letterSpacing:.5,marginBottom:5,display:"flex",alignItems:"center",gap:4}}>
                                    <CheckCircle size={10} color={C.green}/> O QUE ESTÁ INCLUSO
                                  </div>
                                  {sv0.includes.map((item,i)=>(
                                    <div key={i} style={{fontSize:11,color:C.textSec,display:"flex",alignItems:"flex-start",gap:5,marginBottom:3}}>
                        ...
                        {isProtocol&&(
                          <div style={{display:"flex",alignItems:"center",gap:4,marginTop:6}}>
                            ...
                            <span style={{fontSize:11,color:specialty.color,fontWeight:600}}>Ver opções</span>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {selectedGroup.services.map(sv=>{
              ...
              return(
                <button key={sv.id}
                  ...
                  {unavailable&&(
                    <div style={{background:"rgba(234,179,8,0.08)",borderBottom:`1px solid rgba(234,179,8,0.2)`,padding:"5px 14px",fontSize:10,fontWeight:700,color:C.yellow,letterSpacing:.3}}>
                  ...
                  <div style={{padding:"14px 16px",display:"flex",gap:12,alignItems:"flex-start"}}>
                    {/* Ícone / número */}
                    <div style={{width:44,height:44,borderRadius:10,background:specialty.color+"18",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      {sv.exactQty
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,marginBottom:5}}>
                        ...
                        <div style={{textAlign:"right",flexShrink:0}}>
                          {archLabel&&<div style={{fontSize:10,color:C.textSec,marginBottom:1}}>{archLabel}</div>}
                      ...
                      {!sv.planOnly&&serviceType==="planOnly"&&(
                        <div style={{fontSize:10,marginTop:8,fontWeight:600,color:C.textSec}}>
        ...
        {screen==="serviceType"&&specialty&&(
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
              <div style={{width:32,height:32,borderRadius:10,background:specialty.color+"20",display:"flex",alignItems:"center",justifyContent:"center",color:specialty.color}}>
            ...
            {(()=>{
              ...
              if(!anyPlanOnly)return(
                <div style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${C.border}`,borderRadius:14,padding:"16px 16px",display:"flex",gap:14,alignItems:"center",opacity:.45}}>
        ...
        {screen===0&&(
          <Step title="Deseja realizar um pré-planejamento?" sub="Análise prévia de viabilidade antes do planejamento completo." badge="Pré-Plan" badgeColor={C.yellow} onBack={()=>{if(["imp3d","consult"].includes(specialty?.id)){setScreen("specialties");setSpecialty(null);}else if(selectedGroup){setScreen("serviceSubcat");}else{setScreen("services");}}}>
            ...
            <button onClick={()=>setShowPreplanConfirm(true)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,background:C.dark2,border:`1.5px solid ${C.border}`,borderRadius:10,padding:"13px 15px",cursor:"pointer",textAlign:"left",marginBottom:7,color:C.text}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <button onClick={()=>{set("preplan","nao");setScreen(1);}} style={{width:"100%",display:"flex",alignItems:"flex-start",gap:10,background:C.dark2,border:`1.5px solid ${C.border}`,borderRadius:10,padding:"13px 15px",cursor:"pointer",textAlign:"left",color:C.text}}>
              ...
              <div style={{background:C.dark2,border:`1px solid rgba(234,179,8,0.3)`,borderRadius:12,padding:16,display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginBottom:4}}>
                    <span style={{fontSize:10,background:C.yellow,color:"#111",padding:"2px 8px",borderRadius:20,fontWeight:700}}>PRÉ-PLANEJAMENTO</span>
                  </div>
                  <div style={{fontSize:11,color:C.textSec}}>{specialty?.name}</div>
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8,flexShrink:0}}>
                  ...
                  <div style={{display:"flex",gap:6}}>
                    <div style={{position:"relative"}}>
                      ...
                      <div style={{position:"absolute",bottom:"calc(100% + 6px)",right:0,background:"rgba(0,0,0,0.85)",color:"#fff",fontSize:10,fontWeight:600,padding:"4px 8px",borderRadius:6,whiteSpace:"nowrap",pointerEvents:"none",opacity:0,transition:"opacity .15s",zIndex:99}}>Editar</div>
                    </div>
                    <div style={{position:"relative"}}>
                      ...
                      <div style={{position:"absolute",bottom:"calc(100% + 6px)",right:0,background:"rgba(0,0,0,0.85)",color:"#fff",fontSize:10,fontWeight:600,padding:"4px 8px",borderRadius:6,whiteSpace:"nowrap",pointerEvents:"none",opacity:0,transition:"opacity .15s",zIndex:99}}>Remover</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {screen===1&&(
          ...
          ? <Step title="Selecione a(s) arcada(s)" sub="Selecione a(s) arcada(s) do caso." badge="Arcada" badgeColor={C.blue} onBack={()=>setScreen(0)}>
                ...
                {[{key:"sup",label:"Arcada Superior"},{key:"inf",label:"Arcada Inferior"}].map(arc=>{
                  ...
                  return(
                    <button key={arc.key} onClick={()=>setA(a=>({...a,arch:{...a.arch,[arc.key]:!a.arch[arc.key]}}))}
                      ...
                      <div style={{width:22,height:22,borderRadius:6,background:sel?C.blue:C.dark3,border:`2px solid ${sel?C.blue:C.border}`,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:12,fontWeight:800}}>
                        {sel?"✓":""}
                      </div>
                      <div style={{flex:1}}>
              ...
              {(A.arch.sup||A.arch.inf)&&(
                <div style={{background:C.dark3,borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:13,color:C.textSec}}>{[A.arch.sup&&"Superior",A.arch.inf&&"Inferior"].filter(Boolean).join(" + ")}</span>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontSize:11,color:C.textSec,marginBottom:2}}>Subtotal</div>
          ...
          : <Step title="Selecione o(s) dente(s) no odontograma"
              ...
              {(()=>{
                ...
                if(isParcial){
                  ...
                  return(
                    <div style={{background:"rgba(99,102,241,0.06)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:10,padding:"12px 14px",marginBottom:14}}>
                      <div style={{fontSize:11,fontWeight:700,color:C.blue,marginBottom:8,letterSpacing:.5}}>SELEÇÃO DE IMPLANTES</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                        <div style={{background:supValid?"rgba(34,197,94,0.08)":"rgba(255,255,255,0.04)",border:`1px solid ${supValid?"rgba(34,197,94,0.3)":C.border}`,borderRadius:8,padding:"8px 10px",textAlign:"center"}}>
                          <div style={{fontSize:10,color:C.textSec,marginBottom:2}}>Arcada Superior</div>
                          <div style={{fontSize:20,fontWeight:900,color:supValid?C.green:C.textSec}}>{supCount}</div>
                          <div style={{fontSize:10,color:supValid?C.green:C.textSec}}>{supValid?`${fmt(supPrice)}`:"mín. 2"}</div>
                        </div>
                        <div style={{background:infValid?"rgba(34,197,94,0.08)":"rgba(255,255,255,0.04)",border:`1px solid ${infValid?"rgba(34,197,94,0.3)":C.border}`,borderRadius:8,padding:"8px 10px",textAlign:"center"}}>
                          <div style={{fontSize:10,color:C.textSec,marginBottom:2}}>Arcada Inferior</div>
                          <div style={{fontSize:20,fontWeight:900,color:infValid?C.green:C.textSec}}>{infCount}</div>
                          <div style={{fontSize:10,color:infValid?C.green:C.textSec}}>{infValid?`${fmt(infPrice)}`:"mín. 2"}</div>
                        </div>
                      </div>
                      {total>0&&(
                      ...
                      {A.teeth.length===0&&<div style={{fontSize:11,color:C.textSec,textAlign:"center",marginTop:4}}>Selecione os dentes no odontograma (mín. 2 por arcada)</div>}
                    </div>
                  );
                }
                if(service?.exactQty){
                  ...
                  return(
                    <div style={{background:hasSupInf?"rgba(234,179,8,0.08)":total===limit?"rgba(34,197,94,0.08)":"rgba(99,102,241,0.06)",border:`1px solid ${hasSupInf?"rgba(234,179,8,0.35)":total===limit?"rgba(34,197,94,0.3)":"rgba(99,102,241,0.2)"}`,borderRadius:10,padding:"10px 14px",marginBottom:14}}>
                      ...
                      {hasSupInf&&<div style={{fontSize:11,color:C.yellow,display:"flex",alignItems:"center",gap:5}}><AlertTriangle size={11}/> Ambas as arcadas — valor dobrado</div>}
                    </div>
                  );
                }
                return null;
              ...
              {A.teeth.length>0&&(()=>{
                ...
                return(
                  <div style={{background:"rgba(234,179,8,0.06)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:8,padding:"7px 12px",marginBottom:8,fontSize:11,color:C.yellow,display:"flex",alignItems:"center",gap:6}}>
        ...
        {screen==="implantQty"&&service?.usesArch&&(()=>{
          ...
          return(
            <Step title="Tipo de caso" sub={bothArches?"Selecione o tipo de caso para cada arcada.":"Selecione a quantidade de implantes planejada."} badge="Tipo de Caso" badgeColor={C.blue} onBack={()=>setScreen(1)}>
                  ...
                  {[{arc:"sup",label:"Superior"},{arc:"inf",label:"Inferior"}].map(({arc,label})=>(
                    <div key={arc} style={{display:"flex",flexDirection:"column",gap:8}}>
                      ...
                      <div style={{background:`rgba(99,102,241,0.08)`,border:"1px solid rgba(99,102,241,0.2)",borderRadius:8,padding:"6px 10px",textAlign:"center"}}>
                        <div style={{fontSize:10,fontWeight:700,color:C.blue,letterSpacing:.5}}>ARCADA {label.toUpperCase()}</div>
                        {getVal(arc)&&<div style={{fontSize:11,color:C.blue,fontWeight:600,marginTop:1}}>{opts.find(o=>o.qty===getVal(arc))?.label}</div>}
                      </div>
                      {/* 3 opções empilhadas */}
                      {opts.map(opt=>{
                        ...
                        return(
                          <button key={opt.qty} onClick={()=>setVal(arc,opt.qty)}
                            style={{width:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3,background:sel?"rgba(99,102,241,0.12)":C.dark2,border:`2px solid ${sel?C.blue:C.border}`,borderRadius:10,padding:"12px 8px",cursor:"pointer",color:C.text,transition:"border-color .15s,background .15s",position:"relative"}}>
                            {sel&&<div style={{position:"absolute",top:5,right:5,width:14,height:14,borderRadius:"50%",background:C.blue,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"#fff",fontWeight:800}}>✓</div>}
                            <div style={{fontSize:13,fontWeight:900,color:sel?C.blue:"#fff"}}>{opt.label}</div>
                            <div style={{fontSize:10,color:C.textSec}}>{opt.sub}</div>
                          </button>
                        );
                      })}
                    </div>
              ...
              {/* Resumo */}
              {isReady&&(
                <div style={{background:"rgba(99,102,241,0.06)",border:"1px solid rgba(99,102,241,0.15)",borderRadius:10,padding:"10px 14px",marginBottom:14}}>
                  {bothArches
                    ?<>
          ...
          <Step title="Serviço complementar" sub="Opcional. Adicione o planejamento digital do sorriso para orientar o posicionamento dos implantes." badge="Serviço Complementar" badgeColor={C.blue} onBack={()=>isParcial?setScreen(1):setScreen("implantQty")}>
            ...
            <div style={{marginBottom:16}}>
              <button onClick={()=>setA(a=>({...a,dsd3dAddon:!a.dsd3dAddon}))}
                style={{width:"100%",display:"flex",flexDirection:"column",background:A.dsd3dAddon?"rgba(34,197,94,0.06)":C.dark2,border:`2px solid ${A.dsd3dAddon?C.green:C.border}`,borderRadius:14,padding:0,cursor:"pointer",textAlign:"left",overflow:"hidden",transition:"border-color .15s,background .15s"}}>
                <div style={{height:3,background:A.dsd3dAddon?C.green:"#EA580C",width:"100%"}}/>
                <div style={{padding:"14px 16px",display:"flex",gap:12,alignItems:"flex-start"}}>
                  {/* Checkbox */}
                  <div style={{width:24,height:24,borderRadius:7,background:A.dsd3dAddon?C.green:C.dark3,border:`2px solid ${A.dsd3dAddon?C.green:C.border}`,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",color:"#111",fontSize:13,fontWeight:800,marginTop:2}}>
                    {A.dsd3dAddon?"✓":""}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    {/* Nome + preço */}
                    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8,marginBottom:6}}>
                      <div>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:6,marginTop:3,flexWrap:"wrap"}}>
                          <div style={{fontSize:11,color:"#EA580C",fontWeight:600}}>Estética Dental · Somente Planejamento</div>
                          {isParcial&&(
                            <span style={{fontSize:9,fontWeight:800,background:"rgba(34,197,94,0.12)",color:C.green,padding:"2px 8px",borderRadius:20,border:"1px solid rgba(34,197,94,0.3)",letterSpacing:.3}}>RECOMENDADO</span>
                          )}
                          {service?.id==="proto_parc"&&(
                            <span style={{fontSize:9,fontWeight:800,background:"rgba(229,34,41,0.15)",color:C.red,padding:"2px 8px",borderRadius:20,border:"1px solid rgba(229,34,41,0.3)",letterSpacing:.3}}>ALTAMENTE RECOMENDADO</span>
                          )}
                          {service?.id==="proto_total"&&(
                            <span style={{fontSize:9,fontWeight:800,background:"rgba(34,197,94,0.12)",color:C.green,padding:"2px 8px",borderRadius:20,border:"1px solid rgba(34,197,94,0.3)",letterSpacing:.3}}>RECOMENDADO</span>
                          )}
                        ...
                        {isParcial&&(
                          <div style={{fontSize:11,color:C.yellow,background:"rgba(234,179,8,0.06)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:8,padding:"6px 10px",marginTop:8,lineHeight:1.5}}>
                            ⭐ Recomendado para casos que envolvam dentes anteriores superiores e que tenham alta demanda estética.
                          </div>
                        )}
                      </div>
                      <div style={{textAlign:"right",flexShrink:0}}>
                        <div style={{fontSize:11,color:C.textSec,marginBottom:2}}>adicional</div>
            ...
            {/* Resumo */}
            {A.dsd3dAddon&&(
              <div style={{background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:10,padding:"10px 14px",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        {screen===2&&(
          <Step title="Qual a marca do implante?" sub="O kit e o modelo serão filtrados pela marca selecionada." badge="Marca do Implante" badgeColor={C.blue} onBack={()=>{
              ...
              {BRANDS.map(b=>{
                ...
                return(
                  <button key={b} onClick={()=>setA(a=>({...a,brand:b,kit:null,model:null,modelText:""}))}
                    style={{background:sel?"rgba(229,34,41,0.12)":C.dark2,border:`1.5px solid ${sel?C.red:C.border}`,borderRadius:10,padding:"13px 13px",cursor:"pointer",textAlign:"left",color:sel?C.red:C.text,transition:"border-color .15s,background .15s"}}>
                    <div style={{fontWeight:700,fontSize:14,marginBottom:4}}>{b}</div>
                    <div style={{fontSize:11,color:C.textSec}}>{kitsFor(b).length} kit(s)</div>
                  </button>
                );
              })}
            </div>
            ...
            {A.brand==="STRAUMANN"&&(
              <div style={{background:"rgba(234,179,8,0.06)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:10,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:8}}>
                <AlertTriangle size={14} color={C.yellow}/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
              {kitsFor(A.brand).map(kit=>(
                <button key={kit} onClick={()=>setA(a=>({...a,kit,model:null,modelText:""}))}
                  style={{background:A.kit===kit?"rgba(229,34,41,0.12)":C.dark2,border:`1.5px solid ${A.kit===kit?C.red:C.border}`,borderRadius:10,padding:"13px 12px",cursor:"pointer",textAlign:"left",color:C.text,transition:"border-color .15s,background .15s"}}>
                  <div style={{fontWeight:700,fontSize:13,color:A.kit===kit?C.red:C.text,marginBottom:3}}>{kit}</div>
                  <div style={{fontSize:11,color:C.textSec}}>{modelsFor(A.brand,kit).length} modelo(s)</div>
                  {A.kit===kit&&<div style={{fontSize:10,color:C.red,fontWeight:700,marginTop:4}}>✓ Selecionado</div>}
                </button>
              ))}
            </div>
              ...
              ?<><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
                {modelsFor(A.brand,A.kit).map(model=>(
                  <button key={model} onClick={()=>set("model",model)}
                    style={{background:A.model===model?"rgba(229,34,41,0.12)":C.dark2,border:`1.5px solid ${A.model===model?C.red:C.border}`,borderRadius:10,padding:"13px 12px",cursor:"pointer",textAlign:"left",color:A.model===model?C.red:C.text,fontWeight:A.model===model?700:400,fontSize:13,transition:"border-color .15s,background .15s"}}>
                    <div style={{fontWeight:700,fontSize:13,color:A.model===model?C.red:C.text,marginBottom:3,wordBreak:"break-word"}}>{model}</div>
                    {A.model===model&&<div style={{fontSize:10,color:C.red,fontWeight:700,marginTop:4}}>✓ Selecionado</div>}
                  </button>
                ))}
              </div></>
        ...
        {screen===5&&service?.id==="stackable"&&(
          <Step title="Serviços adicionais" sub="O Stackable Guide já contempla todos os serviços adicionais." badge="Serviços Adicionais" badgeColor={C.green} onBack={()=>setScreen(4)}>
            ...
            <div style={{padding:"11px 14px",background:C.dark3,borderRadius:10,display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          ...
          <Step title="Serviços adicionais" sub={service?.usesArch?"Selecione os serviços adicionais para o protocolo.":"Selecione quais dentes receberão cada serviço adicional."} badge="Serviços Adicionais" badgeColor={C.green} onBack={()=>setScreen(4)}>
            ...
            {serviceType==="planOnly"&&(
              <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
                <span style={{fontSize:11,fontWeight:700,background:"rgba(234,179,8,0.12)",color:C.yellow,padding:"4px 12px",borderRadius:20,border:"1px solid rgba(234,179,8,0.3)",letterSpacing:.3}}>SOMENTE ARQUIVO DIGITAL</span>
                <span style={{fontSize:11,fontWeight:700,background:"rgba(229,34,41,0.08)",color:C.red,padding:"4px 12px",borderRadius:20,border:"1px solid rgba(229,34,41,0.25)",letterSpacing:.3}}>NÃO INCLUI IMPRESSÃO</span>
              </div>
            )}
            {service?.usesArch
                ...
                return compsList.map(c=>{
                  ...
                  return(
                    ...
                    <button onClick={()=>{if(isLocked)return;setA(a=>({...a,comps:{...a.comps,[c.id]:sel?0:archCount}}));}}                      style={{width:"100%",display:"flex",alignItems:"flex-start",gap:12,background:isLocked?"rgba(255,255,255,0.02)":sel?"rgba(34,197,94,0.08)":C.dark2,border:`1.5px solid ${isLocked?"rgba(255,255,255,0.07)":sel?C.green:C.border}`,borderRadius:12,padding:"16px 16px",cursor:isLocked?"not-allowed":"pointer",textAlign:"left",marginBottom:isLocked?6:10,color:C.text,opacity:isLocked?0.5:1}}>
                      <div style={{width:22,height:22,borderRadius:6,background:sel?C.green:C.dark3,border:`2px solid ${sel?C.green:C.border}`,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",color:sel?"#111":"transparent",fontSize:12,fontWeight:800,marginTop:1}}>{sel?"✓":""}</div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:14,fontWeight:700,color:isLocked?C.textSec:sel?C.green:C.text,marginBottom:3}}>{c.label}</div>
                        <div style={{fontSize:11,color:C.textSec,lineHeight:1.5}}>{c.desc}</div>
                        {sel&&archCount>1&&(
                      <div style={{marginTop:6}}>
                        <div style={{fontSize:11,color:C.green,fontWeight:600,marginBottom:4}}>Selecionado para {archCount} arcadas:</div>
                        <div style={{display:"flex",gap:6}}>
                          {A.arch.sup&&<div
                        ...
                        <div style={{fontSize:9,color:C.textSec,marginTop:4}}>Toque para ajustar por arcada</div>
                      </div>
                    )}
                      </div>
                      <div style={{textAlign:"right",flexShrink:0}}>
                        ...
                        <div style={{fontSize:10,color:C.textSec,marginTop:2}}>por arcada</div>
                      </div>
                    </button>
                    {/* Aviso proto_parc — trocar para Stackable Guide */}
                    {unavailableParc&&(
                      <div style={{background:"rgba(99,102,241,0.05)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:10,marginBottom:10,overflow:"hidden"}}>
                        ...
                        <button onClick={()=>{
                          ...
                          <div style={{width:36,height:36,borderRadius:10,background:"rgba(99,102,241,0.12)",border:"1px solid rgba(99,102,241,0.25)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{fontSize:11,color:C.textSec}}>Prótese Provisória já inclusa no serviço</div>
                          </div>
                    ...
                    {locked&&(
                      <div style={{background:"rgba(234,179,8,0.06)",border:"1px solid rgba(234,179,8,0.2)",borderRadius:10,marginBottom:10,overflow:"hidden"}}>
                        <div style={{display:"flex",alignItems:"flex-start",gap:8,padding:"10px 12px"}}>
                        ...
                        <button onClick={()=>setA(a=>({...a,dsd3dAddon:true}))}
                          ...
                          <div style={{width:36,height:36,borderRadius:10,background:"rgba(234,179,8,0.1)",border:"1px solid rgba(234,179,8,0.25)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{fontSize:11,color:C.textSec}}>Somente Planejamento · integrado ao planejamento cirúrgico</div>
                          </div>
                          <div style={{textAlign:"right",flexShrink:0}}>
                      ...
                      <div key={c.id} style={{background:isActive?"rgba(34,197,94,0.06)":C.dark2,border:`1.5px solid ${isActive?C.green:C.border}`,borderRadius:12,overflow:"hidden",display:"flex",flexDirection:"row",minHeight:130}}>
                        ...
                        <div style={{flex:1,padding:"12px 10px",display:"flex",flexDirection:"column",justifyContent:"space-between",minWidth:0}}>
                          <div>
                            ...
                            <div style={{fontSize:11,fontWeight:800,color:isActive?C.green:C.textSec}}>
                              {isActive?`+${fmt(c.price*selTeeth.length)}`:`+${fmt(c.price)}`}
                            </div>
                            <div style={{fontSize:10,color:C.textSec}}>{c.desc}</div>
                          </div>
                        ...
                        {caseteeth.length>0?(
                          <div style={{borderLeft:`1px solid ${C.border}`,padding:"10px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:5,minWidth:64,background:"rgba(0,0,0,0.15)"}}>
                            <div style={{fontSize:8,fontWeight:700,color:C.textSec,letterSpacing:.5,textAlign:"center",lineHeight:1.3,marginBottom:2}}>SELECIONE{"\n"}O DENTE</div>
                            {caseteeth.map(tooth=>{
                              ...
                              return(
                                ...
                                style={{width:44,height:32,borderRadius:8,border:`1.5px solid ${isSel?C.green:C.border}`,background:isSel?"rgba(34,197,94,0.2)":C.dark3,cursor:"pointer",fontSize:11,fontWeight:isSel?800:500,color:isSel?C.green:C.textSec,display:"flex",alignItems:"center",justifyContent:"center",transition:"all .12s"}}>
                                  {tooth}
                                </button>
                              );
                            })}
                          ...
                          <div style={{borderLeft:`1px solid ${C.border}`,padding:"10px",display:"flex",alignItems:"center",justifyContent:"center",minWidth:64,background:"rgba(0,0,0,0.15)"}}>
                            <span style={{fontSize:9,color:C.textMut,textAlign:"center",lineHeight:1.3}}>Selecione dentes no odontograma</span>
                          </div>
                        )}
                      </div>
                    );
            ...
            <div style={{margin:"4px 0 10px",padding:"11px 14px",background:C.dark3,borderRadius:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{fontSize:13,color:C.textSec}}>Subtotal estimado</span>
              <span style={{fontSize:18,fontWeight:900,color:C.red}}>{fmt(calcTotal(A))}</span>
            </div>
          ...
          <Step title="Arquivo preparado para impressão" sub="Receba o arquivo .chitubox com peças e suportes configurados." badge="Arquivo p/ Impressão" badgeColor={"#0D9488"} onBack={()=>isStackable?setScreen(4):setScreen(5)}>
            ...
            <div style={{background:"rgba(13,148,136,0.06)",border:"1px solid rgba(13,148,136,0.25)",borderRadius:12,padding:"16px",marginBottom:16}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                <div style={{width:36,height:36,borderRadius:10,background:"rgba(13,148,136,0.15)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
        ...
        {screen===6&&(
          <Step title="Prazo de planejamento" sub="A urgência afeta apenas o planejamento digital. Impressão e frete mantém seus prazos normais após aprovação do planejamento." badge="Prazo" badgeColor={C.yellow} onBack={()=>isStackable?setScreen(4):setScreen(5)}>
            <div style={{fontSize:11,color:C.textSec,marginBottom:14,lineHeight:1.6,background:"rgba(234,179,8,0.06)",border:"1px solid rgba(234,179,8,0.15)",borderRadius:10,padding:"10px 13px"}}>
        ...
        {screen===7&&(
          <div>
            ...
            <span style={{display:"inline-block",fontSize:10,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",background:A.preplan==="sim"?"rgba(234,179,8,0.15)":"rgba(99,102,241,0.15)",color:A.preplan==="sim"?C.yellow:C.blue,padding:"3px 10px",borderRadius:20,marginBottom:10}}>{A.preplan==="sim"?"PRÉ-PLANEJAMENTO":"INFORMAÇÕES DO CASO"}</span>
            ...
            <div style={{marginBottom:20}}>
              <label style={{fontSize:11,fontWeight:700,color:C.textSec,display:"block",marginBottom:6,letterSpacing:.5}}>OBSERVAÇÕES E ORIENTAÇÕES {A.preplan==="sim"?<span style={{color:C.red,fontWeight:700}}>(obrigatório)</span>:<span style={{color:C.textMut,fontWeight:400}}>(opcional)</span>}</label>
              <textarea value={A.observacoes} onChange={e=>set("observacoes",e.target.value)}
                ...
                rows={7} style={{width:"100%",background:C.dark2,border:`1px solid ${A.preplan==="sim"&&!A.observacoes.trim()?"rgba(229,34,41,0.4)":C.border}`,borderRadius:10,padding:"11px 14px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",resize:"vertical",lineHeight:1.7,fontFamily:"inherit"}}/>
              {A.preplan==="sim"&&!A.observacoes.trim()&&<div style={{fontSize:11,color:C.red,marginTop:6}}>Preencha as informações do caso para continuar.</div>}
            </div>
                ...
                <button onClick={()=>setFreight({method:isSaoLuis?"local":"sedex",value:0,prazo:isSaoLuis?"Combinado com a equipe D-CAD":"1 a 2 dias úteis",loading:false,simulated:false})}
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                      ...
                      <span style={{fontSize:10,fontWeight:700,background:"rgba(34,197,94,0.15)",color:C.green,padding:"2px 8px",borderRadius:20}}>GRÁTIS</span>
                    </div>
                    <div style={{fontSize:12,color:C.textSec}}>
                      {isSaoLuis
                  ...
                  <div>
                    ...
                    {[
                      ...
                      <button key={key}
                        ...
                        <div style={{width:36,height:36,borderRadius:10,background:"rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:C.text}}>{icon}</div>
                        <div style={{flex:1}}>
                          <div style={{fontWeight:800,fontSize:13}}>{label}</div>
                          <div style={{fontSize:11,color:C.textSec,marginTop:1}}>{sub}</div>
                          <div style={{fontSize:10,color:C.blue,fontWeight:600,marginTop:2}}>Entrega estimada: {eta} após aprovação</div>
                        </div>
                        <div style={{textAlign:"right",flexShrink:0}}>
          ...
          <div>
            ...
            {services.map((sv,i)=>(
              <div key={sv.id} style={{background:C.dark2,border:`1px solid ${C.border}`,borderRadius:12,padding:16,display:"flex",flexDirection:"column",gap:0}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,gap:8}}>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",marginBottom:4}}>
                      <span style={{fontSize:10,background:C.red,color:"#fff",padding:"2px 8px",borderRadius:20,fontWeight:700}}>SERVIÇO</span>
                      {discount>0&&<span style={{fontSize:11,fontWeight:700,background:"rgba(34,197,94,0.15)",color:C.green,padding:"2px 7px",borderRadius:20}}>15% parceiro</span>}
                      <div style={{display:"inline-flex",alignItems:"center",gap:4,background:sv.serviceType==="print"?"rgba(34,197,94,0.1)":"rgba(234,179,8,0.1)",borderRadius:20,padding:"1px 7px"}}>
                        <span style={{fontSize:11,fontWeight:700,color:sv.serviceType==="print"?C.green:C.yellow}}>{sv.serviceType==="print"?"+ Impressão 3D":"Arquivo Digital (STL)"}</span>
                      </div>
                    </div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8,flexShrink:0}}>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      <div style={{position:"relative"}}>
                        ...
                        <div style={{position:"absolute",bottom:"calc(100% + 6px)",right:0,background:"rgba(0,0,0,0.85)",color:"#fff",fontSize:10,fontWeight:600,padding:"4px 8px",borderRadius:6,whiteSpace:"nowrap",pointerEvents:"none",opacity:0,transition:"opacity .15s",zIndex:99}}>Editar serviço</div>
                      </div>
                      <div style={{position:"relative"}}>
                        ...
                        <div style={{position:"absolute",bottom:"calc(100% + 6px)",right:0,background:"rgba(0,0,0,0.85)",color:"#fff",fontSize:10,fontWeight:600,padding:"4px 8px",borderRadius:6,whiteSpace:"nowrap",pointerEvents:"none",opacity:0,transition:"opacity .15s",zIndex:99}}>Remover serviço</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <div style={{textAlign:"right"}}>
                      ...
                      <div style={{fontSize:10,color:C.green,fontWeight:600,marginTop:2}}>Você economizou {fmt(couponDiscount)}</div>
                    </div>
                  </div>
                </>
              ):(
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <button onClick={()=>{
                  ...
                  setFreightOptions(null);
                  setScreen("specialties");
                }} style={{width:"100%",padding:"13px 20px",background:"transparent",border:`1px solid ${C.border}`,borderRadius:10,color:C.text,fontSize:14,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
  ...
  const renderRow=teeth=>(
    <div style={{display:"flex",gap:3,justifyContent:"center"}}>
      {teeth.map(n=>{const sel=selected.includes(n);return(
        <button key={n} onClick={()=>onToggle(n)} style={{width:32,height:34,borderRadius:5,background:sel?C.red:C.dark3,border:`1px solid ${sel?C.red:C.border}`,color:sel?"#fff":C.textSec,fontSize:11,fontWeight:sel?700:400,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1}}>
...
function CouponBlock({totalAll,onApply}){
  ...
  return(
    <div style={{background:C.dark2,border:`1px solid ${applied?"rgba(34,197,94,0.3)":C.border}`,borderRadius:12,padding:"14px 16px",marginBottom:14,transition:"border-color .2s"}}>
      <div style={{fontSize:11,fontWeight:700,color:C.textSec,letterSpacing:1,marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
      ...
      {!applied?(
        <div>
          <div style={{display:"flex",gap:8}}>
            <input value={code} onChange={e=>{setCode(e.target.value.toUpperCase());setError("");}}
              ...
              style={{flex:1,background:C.dark3,border:`1px solid ${error?C.red:C.border}`,borderRadius:9,padding:"10px 13px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",letterSpacing:1,fontWeight:600,transition:"border-color .2s"}}/>
            <button onClick={handleApply} disabled={loading||!code.trim()}
              style={{padding:"0 16px",background:loading||!code.trim()?"#2a2a2a":C.red,border:"none",borderRadius:9,color:loading||!code.trim()?C.textSec:"#fff",fontSize:13,fontWeight:700,cursor:loading||!code.trim()?"not-allowed":"pointer",flexShrink:0,transition:"all .15s"}}>
              {loading?"...":"Aplicar"}
            </button>
          </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:C.textSec}}>{applied.label} — economia de <b style={{color:C.green}}>{fmt2(applied.discount)}</b></div>
          </div>
          <button onClick={handleRemove} style={{background:"none",border:"none",color:C.textSec,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>Remover</button>
        </div>
      )}
    </div>
  );
  ...
  if(success){
    return(
      <div style={{marginTop:4}}>
        <div style={{background:C.dark2,border:"1px solid rgba(234,179,8,0.35)",borderRadius:14,padding:24,textAlign:"center",marginBottom:16}}>
        ...
        {success.checkout_url&&(
          <button onClick={()=>window.open(success.checkout_url,"_blank")}
            style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8,background:"rgba(234,179,8,0.12)",border:"1px solid rgba(234,179,8,0.35)",borderRadius:10,padding:"13px 20px",color:C.yellow,fontSize:14,fontWeight:700,cursor:"pointer",marginBottom:12}}>
            <Clock size={16}/> Finalizar pagamento →
          </button>
        )}
        <div style={{background:"rgba(229,34,41,0.06)",border:"1px solid rgba(229,34,41,0.25)",borderRadius:14,padding:20,textAlign:"center",marginBottom:12}}>
          ...
          <a href={formUrl} target="_blank" rel="noopener noreferrer"
            style={{display:"inline-flex",alignItems:"center",gap:8,background:C.red,color:"#fff",padding:"12px 24px",borderRadius:10,fontWeight:700,fontSize:14,textDecoration:"none"}}>
            Enviar arquivos do caso →
          </a>
        </div>
        {onBackToDashboard&&(
          <button onClick={onBackToDashboard} style={{width:"100%",padding:"11px 0",background:"transparent",border:`1px solid ${C.border}`,borderRadius:10,color:C.textSec,fontSize:13,fontWeight:600,cursor:"pointer"}}>
            ← Voltar ao dashboard
          </button>
        )}
      </div>
            ...
            <button key={m.id} onClick={()=>setMethod(m.id)}
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                  ...
                  <span style={{fontSize:11,fontWeight:700,background:m.tagColor,color:m.tagText,padding:"2px 7px",borderRadius:20,flexShrink:0}}>{m.tag}</span>
                </div>
